...
*                                       PWM INIT()
*
* Description : This function initiates the given pwm generator
*
* Argument(s) : led color
*
* Return(s)   : 0
*
* Caller(s)   : Application.
...
CPU_INT08S PWM_INIT(CPU_INT08U color)
{
  ...
  } else if(color == PWM_GREEN){
    PWM_G_Start();
    
  } else if(color == PWM_BLUE){
    PWM_B_Start();
  }
  return 0;
}
...
*                                       PWM_SET()
*
* Description : This function sets the pwm value for the pwm generator
*
* Argument(s) : led color , pwm value
*
* Return(s)   : 0
*
* Caller(s)   : Application.
...
CPU_INT08S PWM_SET(CPU_INT08U color,CPU_INT08U value)
{
  ...
  if(color == PWM_RED){
    PWM_R_WriteCompare(value);
    
  } else if(color == PWM_GREEN){
    PWM_G_WriteCompare(value);
    
  } else if(color == PWM_BLUE){
    PWM_B_WriteCompare(value);
  }
  return 0;
}
